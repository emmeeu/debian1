...
RUN mkdir  $HOME/.vnc
RUN echo 'Bbsbbs' | vncpasswd -f > $HOME/.vnc/passwd
RUN echo '/bin/env  MOZ_FAKE_NO_SANDBOX=1  dbus-launch xfce4-session'  > $HOME/.vnc/xstartup
RUN chmod 600 $HOME/.vnc/passwd
RUN chmod 755 $HOME/.vnc/xstartup
RUN echo 'whoami ' >>/Bbsbbs.sh
RUN echo 'cd ' >>/Bbsbbs.sh
RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/Bbsbbs.sh
RUN echo 'cd /noVNC-1.2.0' >>/Bbsbbs.sh
RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/Bbsbbs.sh
RUN chmod 755 /Bbsbbs.sh
EXPOSE 8900
CMD  /Bbsbbs.sh
